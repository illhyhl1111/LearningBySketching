### code from https://github.com/yael-vinker/CLIPasso/blob/main/models/loss.py
...
class Loss(nn.Module):
    ...
    def forward(self, sketches, targets, color_parameters, renderer, epoch, points_optim=None, mode="train"):
        ...
        losses_dict = dict.fromkeys(
            self.losses_to_apply, torch.tensor([0.0]).to(args.device))
...
class CLIPLoss(torch.nn.Module):
    ...
    def forward(self, sketches, targets, mode="train"):
        ...
        for n in range(self.NUM_AUGS):
            ...
            sketch_augs.append(augmented_pair[:bs])
        ...
        for n in range(self.NUM_AUGS):
            loss_clip += (1. - torch.cosine_similarity(
                sketch_features[bs*n:bs*(n+1)], self.targets_features, dim=1))
        self.counter += 1
        return loss_clip
        # return 1. - torch.cosine_similarity(sketches_features, self.targets_features)
    ...
    def forward(self, sketch, target, mode="train"):
        ...
        if mode == "train":
            for n in range(self.num_augs):
                ...
                sketch_augs.append(augmented_pair[:bs])
                img_augs.append(augmented_pair[bs:])
        ...
        for layer, w in enumerate(args.clip_conv_layer_weights):
            if w:
